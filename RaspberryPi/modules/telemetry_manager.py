...
class TelemetryData:
    ...
    axis_error: Optional[int] = None

    motor_error: Optional[int] = None

    encoder_error: Optional[int] = None
...
class HighSpeedTelemetryManager:
    ...
    async def start(self):
        ...
        print(f"\U0001F4E1 Starting High-Speed Telemetry Manager @ {self.target_rate_hz} Hz...")
        ...
        self.threads['error_poll'] = threading.Thread(

            target=self._error_polling_thread,

            daemon=True,

            name="Telemetry-ErrorPoll"

        )

        self.threads['error_poll'].start()
    ...
    def _error_polling_thread(self):

        """Poll ODrive error codes periodically (1Hz)"""
        ...
        while self.running:
            ...
            try:
                ...
                # Use the CAN simple node to read error codes

                node = getattr(self.can_manager, 'node', None)
                ...
                if node is not None:
                    ...
                    try:
                        ...
                        axis_error = loop.run_until_complete(node.read_parameter('axis0.error'))

                        motor_error = loop.run_until_complete(node.read_parameter('axis0.motor.error'))

                        encoder_error = loop.run_until_complete(node.read_parameter('axis0.encoder.error'))
                    ...
                    with self.data_lock:
                        ...
                        self.latest_data.axis_error = axis_error

                        self.latest_data.motor_error = motor_error

                        self.latest_data.encoder_error = encoder_error
    ...
    def get_latest_data(self) -> TelemetryData:
        ...
        with self.data_lock:
            ...
            return TelemetryData(
                ...
                axis_error=self.latest_data.axis_error,

                motor_error=self.latest_data.motor_error,

                encoder_error=self.latest_data.encoder_error
