...
class SimpleCANManager:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # Setup CAN bus
            # Default to socketcan on Linux (Raspberry Pi). Allow override via
            # ODRIVE_CAN_BACKEND environment variable ('socketcan' or 'gs_usb').
            backend = os.environ.get('ODRIVE_CAN_BACKEND')
            if backend is None:
                backend = 'socketcan' if platform.system().lower() == 'linux' else 'gs_usb'

            if backend == 'socketcan':
                # For socketcan, bitrate is configured at the OS level (ip link set ...)
                self.bus = can.Bus(interface='socketcan', channel='can0')
            else:
                # GS-USB (legacy / Windows style). Keep existing parameters for backward compatibility.
                # If GS-USB is used on Linux, users may need libusb backend - handled elsewhere.
                self.bus = can.Bus(interface='gs_usb', channel='can0', index=0, bitrate=self.bitrate)
