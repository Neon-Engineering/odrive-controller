    ...
    def run(self):
        def home_motor(index, node_id, direction, pitch):
            ...
            while not self.interrupted:
                ...
                print(f"  Step command: {pos + step_size:.2f}")
                time.sleep(delay_step)
                new_pos = request_latest_feedback(node_id)
                print(f"  Feedback: {new_pos:.2f}")
                if abs(new_pos - pos) < 0.05:
                    ...
                    for j in range(steps + 1):
                        ...
                        time.sleep(0.1)
                    time.sleep(3.0)
                    print(f"Motor {index+1} homing finished, returning {home_pos:.2f}")
                    return home_pos
                pos = new_pos
        ...
        if not self.interrupted:
            #self.home_offsets[2] = request_latest_feedback(self.motor_nodes[2])  # Use startup position
...
class ControlPanel(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.fb_curves = [
            self.plot.plot(pen=pg.mkPen(color=(255, 105, 180)), name="Motor 1"),
            self.plot.plot(pen=pg.mkPen(color=(255, 165, 0)), name="Motor 2"),
            self.plot.plot(pen=pg.mkPen(color=(0, 255, 0)), name="Motor 3")
        ]
        main_layout.addWidget(self.plot, stretch=5)  # Wider plot area

        control_panel = QtWidgets.QVBoxLayout()
        main_layout.addLayout(control_panel, stretch=1)  # Narrower GUI
    ...
    def update_command(self):
        if self.send_button.isChecked():
            ...
            if self.behavior_buttons[0].isChecked():  # Manual Pos
    ...
    def update_plot(self):
        ...
        # self.cmd_vals.append(self.last_cmd - self.home_offsets[0])  # Offset commanded curve
