...
def send_position_command(position, node_id):	# Function to send position command to ODrive
    vel_ff = 0
    torque_ff = 0
...
def request_latest_feedback(node_id):	# Function to request hall effect sensor position from ODrive
    latest_pos = None
...
def enter_closed_loop(node_id):	# Function to put ODrive in closed-loop-control (enable PID loop)
...
def exit_closed_loop(node_id):	# Function to pull ODrive out of closed-loop-control (disable PID loop)
...
# === DEFINE CONTROL PANEL FOR PLOT AND GUI ===
class ControlPanel(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("JTLA CONTROL PANEL")
        ...
        self.error_timer.timeout.connect(self.monitor_odrive_errors)
        self.error_timer.setSingleShot(False)
    ...
    """
    def monitor_odrive_errors(self):
        new_status_lines = []
        has_error = False

        for node_id in [0, 1, 2]:
            err = self.request_odrive_error(node_id)
            if err is None:
                new_status_lines.append(f"ODrive {node_id}: No Response")
                has_error = True
            elif err == 0:
                new_status_lines.append(f"ODrive {node_id}: OK")
            else:
                new_status_lines.append(f"ODrive {node_id}: ERROR 0x{err:08X}")
                has_error = True

        new_status_text = '\n'.join(new_status_lines)

        # Only update if there's an error or the message changed
        if has_error or new_status_text != self.status_box.text():
            self.status_box.setText(new_status_text)

    def request_odrive_error(self, node_id):
        arb_id = (node_id << 5) | 0x00  # Get_Error command
        msg = can.Message(arbitration_id=arb_id, data=[], is_extended_id=False, is_remote_frame=True)
        ...
        try:
            ...
            while time.time() - start < 0.01:
                response = self.bus.recv(timeout=0.020)
                if not response:
                    break
                if response.arbitration_id == arb_id and not response.is_remote_frame:
                    axis_error = int.from_bytes(response.data[0:4], byteorder='little')
                    return axis_error
        except can.CanError:
            pass
...
if __name__ == "__main__":
    #for node in [0, 1, 2]:			# Uncomment these when actuator is hooked back up
