...
# LVDT linear scaling
LVDT_v1 = 1.164	# Bottom end voltage [volt] (measured using JTLA_LVDTReadout_v03sm)
LVDT_x1 = -2.546	# Bottom end position [in] (measured using calipers)
LVDT_v2 = 2.980		# Top end voltage [volt] (measured using JTLA_LVDTReadout_v03sm)
LVDT_x2 = 2.628		# Top end position [in] (measured using calipers)
LVDT_x0 = -5.862 		# Scaled linear standoff position relative to v=0 position
...
def read_lvdt():
    ...
    if abs(LVDT_position) > 2.70:		# ***LVDT OVERTRAVEL CHECK***
    ...
    def run(self):
        def home_motor(index, node_id, direction, pitch):
            ...
            if node_id == 0:		# When homing motor 1
                ...
                time.sleep(1.5) # Give motor 3 time to reach homing position
...
#         control_panel.addWidget(self.home_button)		***UNCOMMENT TO ADD A RESET BUTTON FOR THE CONTROLLERS***
    ...
    def exit_application(self):
        ...
        # Retract actuator on shutdown 
        motor_active = 3                                                        # Use all 3x motors
        act_length_target = -2.25                                                # Retraction for travel [in]
        act_contribute = act_length_target / motor_active						# Equal stroke contributions
        for j in range(21):		                                        # Use this to slow down how quickly its sent home
            self.rev_targets = [
                self.home_offsets[0] - (act_contribute / Pitch_12) * (j / 20),					# Motor 1 target position
                self.home_offsets[1] - (act_contribute / (Pitch_12 + Pitch_23)) * (j / 20),	    # Motor 2 target position
                self.home_offsets[2] + (act_contribute / Pitch_23) * (j / 20),	]				# Motor 3 target position
            for i in range(3):
                send_position(self.rev_targets[i], self.motor_nodes[i])
            time.sleep(.050)
        time.sleep(1.5)
