...
def read_lvdt():
    ...
    # Perform scaling and offset to go from voltage to inches
        # y = mx + b_standoff + b_home
            # y: position [in]
            # m: slops
            # x: voltage
            # b: offset 
    LVDT_position = ((LVDT_x2-LVDT_x1)/(LVDT_v2-LVDT_v1))*LVDT_voltage + LVDT_x0 - LVDT_xHome	# LVDT position from HOME
    print(f"LVDT Position: {LVDT_position}")
    if abs(LVDT_position) > 2.25:		# ***LVDT OVERTRAVEL CHECK***
        print(f"ABORT: LVDT Overtravel")
        for node in [0, 1, 2]:					
...
class HomingThread(QtCore.QThread):
    ...
    def run(self):
        def home_motor(index, node_id, direction, pitch):
            ...
            delay_step = 0.5				# Adjust delay between homing steps
            backoff_dist = 3.9 / pitch
            LVDT_homing_pos = -1.5	# Retracted lvdt position [in] during m1 homing
            ...
            if node_id == 0:		# When homing motor 1
                LVDT_startup_pos = read_lvdt()  # Read startup position of LVDT
                
                # Send large ball nut to homing position using LVDT and motor 3
                for j in range(21):		                                        # Use this to slow down how quickly its sent home
                    interp_pos = (LVDT_startup_pos*(-1)+LVDT_homing_pos)/Pitch_23 * (j / 20)
                    send_position(interp_pos, 2)	                        # Send motor 3 to homing position
                    time.sleep(0.05)                   
                print(f"Motor 3 sent to homing location")
                print(f"M3 homing pos: {interp_pos}")
                m3_homing_pos = interp_pos
                time.sleep(2.0) # Give motor 3 time to reach homing position

                # Home small ball nut using motors 1 and 3
                while not self.interrupted:
                    print(f"Homing small nut: No contact yet...")
                    send_position(pos + step_size, node_id)						        # Motor 1 target position
                    send_position(m3_homing_pos + (LVDT_homing_pos-read_lvdt())/Pitch_23, 2)#step_size*(Pitch_23/Pitch_12), 2)	            # Motor 3 target position
                    time.sleep(delay_step)
                    new_pos_1 = request_position(node_id)
                    new_pos_3 = request_position(2)
                    if abs(new_pos_1 - pos) < 0.05:                                       # Check if hardstop has been hit
                        print(f"Homing small nut: Contact made...")
                        hardstop = new_pos_1
                        home_pos = hardstop - direction * backoff_dist
                        for j in range(21):		                                        # Use this to slow down how quickly its sent home
                            if self.interrupted:
                                return
                            interp_pos_1 = hardstop + (home_pos - hardstop) * (j / 20)
                            interp_pos_3 = m3_homing_pos - ((backoff_dist*Pitch_12)/Pitch_23) * (j / 20)
                            send_position(interp_pos_1, node_id)	                        # Send motor 1 home
                            send_position(interp_pos_3, 2)	        # Send motor 3 home (could also calculate home position using LVDT)
                            time.sleep(0.050)                           
                        time.sleep(2.0)
                        return home_pos
                    pos = new_pos_1 #pos + step_size
                    m3_homing_pos = new_pos_3 #new_m3_homing_pos + step_size*(Pitch_23/Pitch_12)
                    
            if node_id == 2:		                     # Including this loop for the sole purpose of returning motor 3 home position
                new_pos_3 = request_position(node_id)
                LVDT_current_pos = read_lvdt()
                for j in range(21):		                                        # Use this to slow down how quickly its sent home
                    interp_pos_3 = new_pos_3 - (LVDT_current_pos/Pitch_23) * (j / 20)
                    send_position(interp_pos_3, node_id)	                        # Send motor 1 home
                    time.sleep(0.050)                           
                time.sleep(2.0)
                home_pos = request_position(node_id)    # Motor 3 is already at home, set using LVDT before homing motor 1
                return home_pos                         # Return motor 3 home position
            else:   # Can probably just get rid of this whole loop, this is the old homing script for all motors
                while not self.interrupted:
        ...
        # Initiate homing ___
        self.home_offsets[0] = home_motor(0, self.motor_nodes[0], +1, Pitch_12) # Initiate motor 1 homing
        self.home_offsets[2] = home_motor(2, self.motor_nodes[2], -1, Pitch_23) # Initiate motor 3 homing
...
class ControlPanel(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.home_button.setStyleSheet("""QPushButton {background-color: darkblue; color: white; font-size: 15px; font-weight: normal; border-radius: 5px; padding: 10px; }""")
...
#         self.showFullScreen()			# UNCOMMENT FOR FULL SCREEN CONTROL PANEL
    ...
    def handle_reset(self):
        ...
        self.sine_sweep_active = False
        self.last_cmd = 0.0
        self.rev_targets = [0.0, 0.0, 0.0]  # Update for plot
        for i in range(3):
            exit_closed_loop(i)
            enter_closed_loop(i)
        for i in range(3):
    ...
    def handle_behavior_toggle(self):
        if self.behavior_buttons[0].isChecked():
            ...
            self.behavior_buttons[1].setStyleSheet("""QPushButton {background-color: green; color: white; font-weight: normal; border-radius: 5px; padding: 10px; }""")  
    ...
    def exit_application(self):
        ...
        # Retract actuator on shutdown ***UPDATE FOR SLOWER RETRACTION***
        motor_active = 3                                                        # Use all 3x motors
        act_length_target = -1.0                                                # Retraction for travel [in]
        act_contribute = act_length_target / motor_active						# Equal stroke contributions
        self.rev_targets = [
            self.home_offsets[0] - act_contribute / Pitch_12,					# Motor 1 target position
            self.home_offsets[1] - act_contribute / (Pitch_12 + Pitch_23),	    # Motor 2 target position
            self.home_offsets[2] + act_contribute / Pitch_23,	]				# Motor 3 target position
        for i in range(3):
            send_position(self.rev_targets[i], self.motor_nodes[i])
        time.sleep(2.0)
