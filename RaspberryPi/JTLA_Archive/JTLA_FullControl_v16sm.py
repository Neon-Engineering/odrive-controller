...
def read_lvdt():
    ...
    voltage = signed_value * VREF / (2**23)
    # Add something here to scale/offset voltage to linear travel
    print(f"LVDT Voltage: {voltage}")
    if abs(voltage) > 2.2:		# ***LVDT OVERTRAVEL CHECK***
        for node in [0, 1, 2]:					
...
#         self.showFullScreen()			# UNCOMMENT FOR FULL SCREEN CONTROL PANEL
    ...
    def update_button_color(self):
        if self.motor_buttons[0].isChecked():
            self.motor_buttons[0].setStyleSheet("""
                QPushButton {
                    background-color: green;
                    color: white;
                    font-weight: normal;
        ...
        if self.motor_buttons[1].isChecked():
            self.motor_buttons[1].setStyleSheet("""
                QPushButton {
                    background-color: green;
                    color: white;
                    font-weight: normal;
        ...
        if self.motor_buttons[2].isChecked():
            self.motor_buttons[2].setStyleSheet("""
                QPushButton {
                    background-color: green;
                    color: white;
                    font-weight: normal;
