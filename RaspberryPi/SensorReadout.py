# This script was adapted from JTLA_LVDTReadout_v03sm.py to read LVDT position data
...
ADC.ADS1263_SetMode(0)  # Differential mode
...
def read_adc():
    ...
    while True:
        ...
        # Print scale and offset position of LVDT relative to HOME
        LVDT_position = ((LVDT_x2-LVDT_x1)/(LVDT_v2-LVDT_v1))*LVDT_voltage + LVDT_x0 - LVDT_xHome
        print(f"LVDT position: {LVDT_position}")

        current_time = time.time() - start_time
        time_data.append(current_time)
        voltage_data.append(LVDT_voltage)
        position_data.append(LVDT_position)
        time.sleep(UPDATE_INTERVAL / 1000.0)

# === Plot setup ===
fig, ax = plt.subplots()
line, = ax.plot([], [], lw=2)
ax.set_title('Live LVDT Position Readout')
